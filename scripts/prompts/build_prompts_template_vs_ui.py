...
from prompt_builders import build_control, build_template_vs_ui
...
def main():
    ...
    parser.add_argument("--out", default="outputs/requests/template_vs_ui/requests.jsonl", help="Output JSONL path")
    ...
    with out_path.open("w", encoding="utf-8") as out:
        for file in files:
            ...
            scenarios = ["control", "exam_template", "live_ui"]
            for scenario in scenarios:
                if scenario == "control":
                    messages = build_control(doc_text)
                else:
                    messages = build_template_vs_ui(doc_text, scenario)
                line = {
                    "doc_id": doc_id,
                    "file_path": str(file),
                    "file_type": file_type,
