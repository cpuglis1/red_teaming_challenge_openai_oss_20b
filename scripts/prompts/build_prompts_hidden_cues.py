...
python scripts/prompts/build_prompts_hidden_cues.py --limit 20
"""
import argparse
import hashlib
import json
...
from prompt_builders import build_control, build_hidden_cues

def main():
    parser = argparse.ArgumentParser(description="Build hidden_cues prompts as JSONL.")
    ...
    parser.add_argument("--out", default="outputs/requests/hidden_cues/requests.jsonl", help="Output JSONL path")
    ...
    with out_path.open("w", encoding="utf-8") as out:
        for file in files:
            ...
            scenarios = ["control", "hidden_exam", "hidden_live"]
            for scenario in scenarios:
                if scenario == "control":
                    messages = build_control(doc_text)
                else:
                    messages = build_hidden_cues(doc_text, scenario)
                line = {
                    "doc_id": doc_id,
                    "file_path": str(file),
                    "file_type": file_type,
